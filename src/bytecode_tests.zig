...
test "compile simple expression to bytecode" {
    ...
    defer mir_module.deinit(allocator);
    
    // Lower to bytecode
    var bytecode_module = try mir_to_bytecode.convert(allocator, mir_module);
    defer bytecode_module.deinit(allocator);
    
    // Verify the bytecode structure
    try std.testing.expectEqual(@as(usize, 1), bytecode_module.functions.items.len);
    
    const main_func = bytecode_module.functions.items[0];
