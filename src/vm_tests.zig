...
fn testGeneExecution(source: []const u8, expected: types.Value) !void {
    ...
    const func = try bytecode.lowerToBytecode(allocator, nodes);
    var gene_vm = try vm.VM.init(allocator);
...
test "execute boolean literal" {
    try testGeneExecution("true", .{ .Bool = true });
}
